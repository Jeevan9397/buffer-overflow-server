...
[](const crow::request &req, int id) {
    ...
    for (auto &c : customers) {
        if (c.id == id) {
            ...
            // Allocate extra space: 40 for buffer, 1 for canary, rest for overflow simulation
            char combined[64];
            char* addrBuffer = combined;           // first 40 bytes
            char* canaryPtr = &combined[40];       // next byte
            *canaryPtr = '#';                      // set canary

            // Dangerous: copy more than 41 to simulate overflow
            strncpy(addrBuffer, newAddr.c_str(), 60);  // Overwrites buffer + canary + extra
            addrBuffer[39] = '\0';  // Just force null in visible buffer

            std::string fullDump = std::string(addrBuffer, 40);  // capture up to canary
            fullDump += "[CANARY:" + std::string(1, *canaryPtr) + "]";
            fullDump += std::string(&combined[41], 64 - 41);  // show overflow after canary

            log("Off-by-one address buffer + canary + overflow: " + fullDump);
    ...
    try {
        auto js = crow::json::load(req.body);
        if (!js) {
            log("Malformed JSON in /api/customers");
            return crow::response(400, "Invalid JSON");
        }

        std::string name = js["name"].s();
        log("Received POST /api/customers with name length: " + std::to_string(name.size()) + " | value: " + name);

        // Simulate buffer overflow 
        char buffer[32];
//         if (name.size() >= sizeof(buffer)) {
//     log("Rejected input: name too long (" + std::to_string(name.size()) + ")");
//     return crow::response(400, "Name too long. Must be under 32 characters.");
// }
// strncpy(buffer, name.c_str(), sizeof(buffer) - 1);
// buffer[sizeof(buffer) - 1] = '\0';


        strcpy(buffer, name.c_str());  // NO BOUNDS CHECK = VULNERABLE!

        log("Buffer Overflow Content: " + std::string(buffer));
        ...
        Customer c;
        c.id = nid;
        c.name = buffer;  // Use overflowed buffer
        // c.father = js["father"].s();
        char FatherBuf[40];
        std::string father = js["father"].s();
        snprintf(FatherBuf, sizeof(FatherBuf),"%s", father.c_str()); //explicitly specifying the format and treating user input as a string
        ...
        log(std::string("EXCEPTION in POST /api/customers: ") + e.what());
        return crow::response(500, "Internal Server Error: " + std::string(e.what()));
    } catch (...) {
        log("Unknown EXCEPTION in POST /api/customers");
        return crow::response(500, "Unknown server error occurred.");
    }
});
    ...
    CROW_ROUTE(app, "/api/import").methods("POST"_method)([](const crow::request &req)
        ...
        while (std::getline(csvData, line)) {
            ...
            // 🔐 Defensive Check for oversized input
            if (name.length() > 100) {
                log("REJECTED: Overly long name in import: len=" + std::to_string(name.length()));
                continue;  // Skip this row without crashing
            }
                c.id = ++maxId;
                c.name = name;
                c.father = fields[2];
